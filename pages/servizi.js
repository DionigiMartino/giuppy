...
import { Lightbulb, Shield, Paintbrush, ChevronDown, ChevronLeft, ChevronRight, Quote, Star, CheckCircle, UserPlus, Box, Settings, Drill, HeartHandshake } from "lucide-react";
...
const ServicesPage = () => {
  ...
  const services = [
    {
      ...
      description: "Trasformiamo i vostri spazi con controsoffitti all'avanguardia che uniscono estetica e funzionalità.",
      icon: <Lightbulb size={24} />,
      image: "/images/servizi/controsoffitti-moderni.jpg",
    },
    {
      title: "Illuminazione Integrata",
      description: "Creiamo atmosfere uniche con sistemi di illuminazione integrati e personalizzabili.",
      icon: <Lightbulb size={24} />,
      image: "/images/servizi/illuminazione-integrata.jpg",
    },
    {
      title: "Isolamento Acustico",
      description: "Ottimizziamo l'acustica dei vostri ambienti con soluzioni innovative e performanti.",
      icon: <Shield size={24} />,
      image: "/images/servizi/isolamento-acustico.jpg",
    },
    {
      title: "Design Personalizzato",
      description: "Progettiamo soluzioni su misura per soddisfare le vostre esigenze estetiche e funzionali.",
  ...
  const renderHero = () => (
    <section ref={heroRef} className="relative h-screen bg-blue-600 text-white overflow-hidden">
      <motion.div className="absolute inset-0" initial="hidden" animate={heroControls} variants={heroVariants}>
        <Image src="/images/servizi-hero.jpg" alt="Giupy Controsoffitti Servizi" layout="fill" objectFit="cover" quality={100} priority className="opacity-30" />
      </motion.div>
      <div className="relative z-10 container mx-auto px-4 h-full flex flex-col justify-center items-center text-center">
        <motion.h1 className="text-5xl md:text-7xl font-bold mb-6" initial={{ opacity: 0, y: -50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }}>
          I Nostri Servizi
        </motion.h1>
        <motion.p className="text-xl md:text-2xl max-w-2xl mb-8" initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.4 }}>
              ...
              <div className="relative z-10 p-6 h-full flex flex-col justify-between">
                ...
                >
                  <ul className="space-y-2">
                    {['Consulenza personalizzata', 'Progettazione 3D', 'Installazione professionale'].map((feature, i) => (
                      <motion.li 
                        key={i}
 ...
 const renderProcess = () => (
  ...
  const renderFAQ = () => (
    <section className="py-20 bg-gray-100">
      <div className="container mx-auto px-4">
        <motion.h2 className="text-4xl md:text-5xl font-bold text-center mb-16" {...fadeIn}>
          Domande Frequenti
        </motion.h2>
        <div className="max-w-3xl mx-auto">
          {[
            {
              question: "Quanto tempo richiede l'installazione di un controsoffitto?",
              answer: "Il tempo di installazione varia in base alle dimensioni e alla complessità del progetto. In genere, per una stanza standard, l'installazione può richiedere da 1 a 3 giorni. Per progetti più grandi o complessi, potrebbe essere necessario più tempo. Forniamo sempre una stima precisa dei tempi prima di iniziare il lavoro.",
            },
            {
              question: "Che tipo di manutenzione richiedono i vostri controsoffitti?",
              answer: "I nostri controsoffitti sono progettati per richiedere una manutenzione minima. Una pulizia regolare con un panno morbido o un'aspirapolvere è sufficiente nella maggior parte dei casi. Per controsoffitti in aree umide o con esigenze particolari, potremmo consigliare procedure di manutenzione specifiche per garantire la longevità del prodotto.",
            },
            {
              question: "Offrite servizi di consulenza per la scelta del design?",
              answer: "Assolutamente sì! Il nostro team di designer esperti offre un servizio di consulenza completo. Lavoreremo con voi per comprendere le vostre esigenze, preferenze di stile e vincoli di budget, proponendo soluzioni su misura che si adattano perfettamente al vostro spazio.",
            },
            {
              question: "I vostri prodotti sono ecosostenibili?",
              answer: "La sostenibilità è una nostra priorità. Utilizziamo materiali ecologici e processi di produzione rispettosi dell'ambiente ove possibile. Offriamo anche opzioni specifiche di prodotti eco-friendly per i clienti particolarmente attenti all'impatto ambientale.",
            },
          ].map((item, index) => (
            <motion.div
              key={index}
    ...
    <section className="py-20 bg-blue-600 text-white">
      <div className="container mx-auto px-4 text-center">
        ...
        <motion.p
          ...
          viewport={{ once: true }}
        >
          Lasciate che la nostra esperienza e passione diano vita ai vostri progetti. Contattateci oggi stesso per una consulenza gratuita e scoprite come possiamo elevare i vostri interni a nuovi livelli di bellezza e funzionalità.
        </motion.p>
        <motion.div
